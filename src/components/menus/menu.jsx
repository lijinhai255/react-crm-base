...
class CustomMenu extends React.Component {
  ...
  renderMenuItem = ({key, icon, title,},i) => {
    return (
      <Menu.Item key={key}>
        <Link to={key}>
          {icon && <Icon type={icon}/>}
  ...
  renderSubMenu = ({key, icon, title, subs},i) => {
    // console.log(key,subs,'findkey')
    return (
      <Menu.SubMenu key={i} title={<span>{icon && <Icon type={icon}/>}<span>{title}</span></span>}>
        {
          subs && subs.map((item ,i) => {
